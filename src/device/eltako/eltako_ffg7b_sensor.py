...
class EltakoFFG7BSensor(BaseDevice, BaseMqtt, BaseCyclic):
    ...
    def _determine_and_store_since(self, value_enum: _HandleValue):
        success_value = _HandleValue.is_success(value_enum)
        if not self._write_since_no_error or success_value:
            key_state = _StorageKey.VALUE_SUCCESS.value
            key_time = _StorageKey.TIME_SUCCESS.value
        else:
            key_state = _StorageKey.VALUE_ERROR.value
            key_time = _StorageKey.TIME_ERROR.value

        if success_value:
            self._storage.delete(_StorageKey.VALUE_ERROR.value)
            self._storage.delete(_StorageKey.TIME_ERROR.value)
    ...
    def _create_message(self, value: _HandleValue, since: Optional[datetime.datetime],
    ...
    def extract_handle_state(self, value):
        if value == 3:
            return _HandleValue.CLOSED
        elif value == 2:
            return _HandleValue.OPEN
        elif value == 1:
            return _HandleValue.TILTED
        else:
            return _HandleValue.ERROR
    ...
    def process_enocean_message(self, message: EnoceanMessage):
        ...
        try:
            ...
            self._logger.exception(ex)
            value = _HandleValue.ERROR

        if value == _HandleValue.ERROR and self._logger.isEnabledFor(logging.DEBUG):
    ...
    def _restore_last_state(self):
        ...
        last_observation = self._storage.get(_StorageKey.TIME_LAST_OBSERVATION.value)
        if not last_observation:
            return
        diff_seconds = (self._now() - last_observation).total_seconds()
        if diff_seconds > self._restore_last_max_diff:
            return
        if self._storage.get(_StorageKey.TIME_ERROR.value) is not None:
            return

        last_value = self._storage.get(_StorageKey.VALUE_SUCCESS.value)
        last_since = self._storage.get(_StorageKey.TIME_SUCCESS.value)
        if not last_value or not last_since:
            return

        last_handle_value = _HandleValue.parse(last_value)
        if not _HandleValue.is_success(last_handle_value):
            return
    ...
    def close_mqtt(self):
        ...
        self._storage.set(_StorageKey.TIME_LAST_OBSERVATION.value, self._now())
        try:
            self._storage.save()
        except StorageException as ex:
            self._logger.exception(ex)
